...
sys.path.insert(0, BE + "/src")
os.chdir(BE)
import httpx
import pgserver
import redislite

pgdir = pathlib.Path("/tmp/live-pg")
pg = pgserver.get_server(str(pgdir))  # already migrated+seeded
...
env = {
    **os.environ,
    ...
    "REDIS_URL": f"unix://{redis.socket_file}",
    "ENVIRONMENT": "local",
    "LOG_JSON": "false",
    "PYTHONPATH": BE + "/src",
}
...
try:
    print(
        "API  :",
        up("http://127.0.0.1:8000/health/liveness"),
        "| WEB :",
        up("http://127.0.0.1:3000/login"),
        flush=True,
    )
    ...
    print(
        ...
        "| form:",
        "Sign in" in c.get("/login").text,
        flush=True,
    )
    r = c.post(
        "/api/auth/login",
        json={"email": "asha@acme.test", "password": "demo-local-passphrase-2026"},
    )
    print(
        "POST /api/auth/login  ->",
        r.status_code,
        "| cookie:",
        any("airev-session" in k for k in c.cookies.keys()),
        flush=True,
    )
    ...
    r = c.post(
        ...
        json={
            "first_name": "Browser",
            "last_name": "Flow",
            "email": f"bf-{int(time.time())}@example.in",
            "source": "manual",
        },
    )
    ...
    print(
        "GET  /leads/{id}      ->",
        c.get(f"/leads/{lid}").status_code,
        "| name shown:",
        "Browser" in c.get(f"/leads/{lid}").text,
        flush=True,
    )
    r = c.patch(
        f"/api/leads/{lid}",
        headers={"x-csrf-token": csrf, "if-match": f'W/"{ver}"'},
        json={"last_name": "Edited"},
    )
    ...
    c2.post(
        "/api/auth/login",
        json={"email": "ravi@globex.test", "password": "demo-local-passphrase-2026"},
    )
    r = c2.get(f"/leads/{lid}")
    print(
        "tenant B opens A's URL ->",
        r.status_code,
        "| 'Not found':",
        "Not found" in r.text,
        flush=True,
