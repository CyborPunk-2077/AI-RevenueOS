...
| A30 | **P1 correctness** | `DomainError` and its subclasses (`InvalidTransition`, `PolicyViolation`) had no exception handler. Every domain rule the deals policy enforces -- an invalid stage move, a missing loss reason, an unmet required field -- reached the generic handler and returned **500**. That reads as "the server is broken" rather than "that is not allowed", and in production would have paged an on-call engineer for a user error. | mapped to 422 with the domain's own code in `api/middleware/errors.py`; `tests/e2e/test_crm_deals.py::test_moving_to_lost_requires_a_reason` asserts the status explicitly |
| A29 | P2 | `AuditRecorder` had been written and tested at M04 and called from nowhere — the third piece of dead security machinery this audit has found, after `apply_scope` (A2) and `sessions_to_evict` (A19). Contacts and accounts are its first real callers, writing the entry inside the same unit of work as the change. | `tests/e2e/test_crm_contacts_accounts.py::TestOutboxAndAudit` asserts both the outbox row and the audit row exist after a create, and that another tenant cannot read the audit row |
| A22 | **P1 tooling** | `next lint` had no config, so it asked an interactive setup question and **waited**. `turbo run lint` hung instead of failing — the worst outcome in CI or a launcher, because a hang looks like slowness rather than a defect. | `.eslintrc.js` committed; `next lint --max-warnings 0` clean |
| A23 | **P1 tooling** | No `pnpm-lock.yaml` despite `packageManager: pnpm@9.7.0`. Nothing was reproducible, and the web image had been switched to npm to work around it. `pnpm-workspace.yaml` also globbed `packages/*` — six empty directories, nothing tracked in Git — so every install warned about workspace members that do not exist. | lockfile committed (619 packages, ~16s frozen install); workspace narrowed to `apps/*`; Dockerfile back on pnpm |
| A24 | P2 | `make verify` described itself as "everything CI runs on a pull request" while running no frontend test and no frontend build. Two of the four things P0-3 was meant to guarantee were outside the gate that claimed to cover them. | `verify` now includes `test-web` and `build-web` |
| A25 | P2 | Next rewrites `allowJs` and `esModuleInterop` into `tsconfig.json` on first lint or build, so a clean checkout came back dirty after any gate run. | both set explicitly, with `forceConsistentCasingInFileNames`; `strict` unchanged |
| A26 | **P1 operability** | The Windows launcher started four of the nine services: the four worker pools and Beat were never launched, so the outbox relay never ran. The demo looked healthy because nothing in the visible flow needed a worker. Beat also depended on `service_started`, which only proves a process was launched. | launcher starts the full tier and waits on Docker health; Beat waits for `service_healthy`; `tests/e2e/test_local_stack_contract.py` pins both |
| A27 | **P1 data safety** | `-Reset` was a switch on the everyday launcher and removed the `pgdata` volume with no confirmation. | moved to `RESET_DEMO.cmd` / `scripts/reset-demo.ps1`, which requires the word `reset` to be typed; a test asserts the launcher contains no volume-removing command |
| A28 | **P1 correctness** | No `ENCRYPTION_MASTER_KEY` was set anywhere in compose. MFA seals the TOTP secret with envelope encryption and fails closed without a key, so the enrolment added in P0-2 would have returned 422 the first time anyone tried it in the browser. | set for the API and worker pools with a clearly-local value; `assert_production_safe` still refuses to boot outside `local` without a real one |
| A19 | **P1 security** | `sessions_to_evict` had been unit-tested since M04 but was called from nowhere, so `MAX_SESSIONS_PER_USER` was documented and never enforced: a user could accumulate unlimited concurrent refresh-token families, and a stolen one would survive indefinitely alongside the legitimate ones. Same class of defect as A2 (`apply_scope` dead code). | wired into `issue_session`, which every sign-in path funnels through; `tests/e2e/test_auth_surface.py::TestSessions::test_the_session_cap_evicts_the_oldest` opens 12 sessions and asserts the oldest refresh token is dead and the newest still rotates |
| A20 | **P1 security** | Revoking a session dropped its refresh tokens but left the already-issued access token valid for the remainder of its 15-minute TTL, so `DELETE /auth/sessions/{id}` and `logout-all` were promises the system did not keep. | `revoke_access_jtis` blacklists the jti in Redis, which `get_principal` already consulted; asserted by the session-revocation and logout-all tests |
| A21 | P2 | The demo seed reset password, status and lockout counters on re-run but not MFA state, so enrolling an authenticator against a throwaway demo account and losing the device left the demo permanently unusable — login would demand a code nobody could produce. | `seed_demo` now clears `mfa_enabled`, the encrypted secret and the recovery codes |
...
| M06 | Onboarding, templates, plans | **IV** | 8 mandatory + 1 generic template as versioned JSON; `apply_template` preserves customisation and rejects guardrail weakening; 3 plans with documented limits. A versioned tenant-scoped onboarding state machine now provides read, step transition, and completion APIs with dependency validation, durable idempotency, atomic audit/outbox evidence, and explicit non-activation semantics. | External provider activation still requires the separately reported provider readiness prerequisites and approvals. |
...
| M08 | Leads, forms, dedupe, assignment | **P** | `LeadService` (capture, get, update, atomic bulk status/assignment, duplicates, qualify, review, convert) is role-scoped; dedupe uses confidence bands; source events are preserved. `POST /leads/bulk` is bounded, durably idempotent, and atomically audited/outboxed. | Form builder/publish, CSV import, webhook capture, assignment-rule execution, `/deduplicate`, `/merge`, `/disqualify`, and `/restore` remain absent |
...
| M12 | AI gateway, prompts, qualification | **P / XB** | Provider-neutral gateway, pinned models, policy routing, circuit breakers, input/output guards, degradation, usage metering, budget stops, and tool limits. Every task now has immutable Git-backed versioned prompts/gold sets; platform-only sync/evaluate/promote/rollback is durably idempotent and atomically audited/outboxed. Runtime calls fail closed without a promoted prompt. The offline CI evaluator truthfully makes no provider-quality claim. | Approved provider credentials and sandbox model-quality evidence remain external; no live model call is claimed |
| M13 | Documents, extraction, RAG | **P / XB** | Tenant/RBAC/RLS-safe document and file-metadata services, routes and contact/deal UI; state+audit+outbox transaction; ETag edits; 24 real-Postgres tests including cross-tenant denial; file-security suite and disabled-storage browser path verified | AWS is unavailable, so no upload URL or stored-file claim is made. Presign completion, ClamAV transport, extraction/chunking/embedding/retrieval remain gated; see `runbooks/storage-activation.md` |
...
| M20 | Analytics and exports | **P / XB** | Scoped funnel, qualification, source, revenue, payment, appointment, conversation, SLA and assignee metrics; date-range API and UI; idempotent tenant-bound daily rollup job; 9 real-Postgres tests including self/cross-tenant scope. Export intents are step-up/RBAC checked, audited and outboxed | Private CSV materialization/download stays honestly disabled until AWS export storage and its worker transport are activated; no file, key or URL is fabricated |
