...
| 19 | Dashboards give correct funnel, source, SLA, qualification, appointment, payment and team results | Evidenced | `application/analytics/service.py` applies repository scope before every aggregate and returns all required sections; `tests/e2e/test_analytics.py` reconciles the HTTP shape, range limits, self scope, cross-tenant sources and idempotent daily rollups against real PostgreSQL/RLS; the browser renders totals, trends and disabled exports |
| 20 | Export is entitlement and role checked, asynchronous, private, auditable, tenant isolated | Evidenced as disabled | Step-up and `analytics:export` gate creation; a durable blocked intent, immutable audit row and outbox event commit together; cross-tenant reads are 404. With AWS unavailable, `s3_key` remains null and no download URL or placeholder artifact is returned |
