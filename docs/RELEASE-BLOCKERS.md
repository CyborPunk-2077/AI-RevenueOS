...
| P1-2 | **Partial.** Auth, leads, AI, CRM, consent, file downloads, analytics exports, tenant governance, workflow controls/execution, public booking, authorization denials and verified payment transitions are durably audited; provider/config, support and bulk paths remain. Consent grants/withdrawals are immutable, tenant/scoped, audit/outbox atomic, cancel queued delivery and block later sends. | criterion 22 | Finish every remaining `MANDATORY_AUDIT_ACTIONS` path and assert atomic audit rows in E2E | remaining product work |
