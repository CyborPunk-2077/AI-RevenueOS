...
| P1-2 | **Partial.** Auth, leads, AI, CRM, consent, provider/config, file downloads, analytics exports, tenant governance, workflow controls/execution, public booking, authorization denials, verified payment transitions, and support access are durably audited; the bulk-operation path remains. Provider secrets are tenant-envelope-encrypted, never returned or audited, and saved configurations remain truthfully pending until deployment flags and external gates are verified. Support access is tenant-approved, read-only, MFA-step-up protected, time-bound, durably idempotent, RLS-isolated, and revoked fail-closed. | criterion 22 | Finish the bulk-operation audit path with atomic real-Postgres evidence | remaining product work |
