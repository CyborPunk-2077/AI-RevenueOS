...
| P1-2 | **Partial.** Auth, leads, AI, CRM, consent, provider/config, file downloads, analytics exports, tenant governance, workflow controls/execution, public booking, authorization denials and verified payment transitions are durably audited; support and bulk paths remain. Provider secrets are tenant-envelope-encrypted, never returned or audited, and saved configurations remain truthfully pending until deployment flags and external gates are verified. | criterion 22 | Finish support-access and bulk-operation audit paths with atomic real-Postgres evidence | remaining product work |
