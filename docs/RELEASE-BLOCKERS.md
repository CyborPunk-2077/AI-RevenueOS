...
| ~~P2-6~~ | **ENGINEERING RESOLVED 2026-08-04.** The test tree measured 178 errors in 23 files and now reports zero under strict mypy; `tests` joined the mypy `packages` list, so bare `mypy` (CI, `make`, pre-commit) covers 264 source files. Two pre-existing `# type: ignore` suppressions were removed rather than extended. | None. |
| P2-7 | **ENGINEERING RESOLVED 2026-08-04, EXTERNALLY GATED.** OpenTelemetry spans cover the HTTP edge, Celery tasks, outbox dispatch, the AI gateway and outbound webhooks, with W3C context propagated from request to worker. Attributes are allow-listed and scrubbed, exception messages and URLs are never recorded, and tracing is fail-closed off without both `OTEL_ENABLED` and an endpoint. See `docs/runbooks/tracing.md`. | No collector is deployed or verified, `traceparent` from callers stays untrusted until an ingress strips it, and trace retention/access approval is outstanding. No production tracing is claimed. |
| ~~P2-8~~ | **ENGINEERING RESOLVED 2026-08-04.** Storybook 8 (`@storybook/nextjs`) covers the component surface with 14 stories over 5 features, `@storybook/addon-a11y` for interactive checks, and `@storybook/test-runner` + `axe-playwright` scanning every story in Chromium against WCAG 2.1 AA including colour contrast. `pnpm a11y` was previously a no-op because no package declared the task; it now builds Storybook statically and fails on any violation. | None. |
| ~~P2-9~~ | **ENGINEERING RESOLVED 2026-08-04.** `envs/dev`, `envs/staging` and `envs/sandbox` join `envs/prod`, each with its own account, state bucket, lock table and non-overlapping CIDR. Staging mirrors production's topology because the release gates run there; dev and sandbox are deliberately disposable. `backend/tests/contract/test_terraform_environments.py` pins those differences, and CI now runs `terraform fmt -check` and `validate` on every environment instead of `fmt || true`. The network module was missing route tables entirely - added. | No AWS account exists, nothing is applied, no state bucket is created. Gates 4.1-4.8 remain open. |
